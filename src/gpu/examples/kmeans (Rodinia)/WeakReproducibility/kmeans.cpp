...
static cl_device_id   * device_list = NULL;
...
static int initialize(int use_gpu)
{
	...
	if ((result = clGetPlatformIDs(1, &platform_id, NULL)) != CL_SUCCESS) { printf("ERROR: clGetPlatformIDs(1,*,0) failed. Error code: %d\n", result); return -1; }
	...
	num_devices = (int) (size / sizeof(cl_device_id));
 	if( num_devices < 1 ) { printf("ERROR: clGetContextInfo() returned 0 devices\n"); return -1; }
...
cl_mem d_feature = NULL;
cl_mem d_feature_swap = NULL;
cl_mem d_cluster = NULL;
cl_mem d_membership = NULL;

cl_kernel kernel = NULL;
cl_kernel kernel_s = NULL;
cl_kernel kernel2 = NULL;

int   *membership_OCL = NULL;
int   *membership_d = NULL;
float *feature_d = NULL;
float *clusters_d = NULL;
float *center_d = NULL;
...
int allocate(int n_points, int n_features, int n_clusters, float **feature)
{
	...
	const char * program_file = "./kmeans.cl";
	FILE *program_handle;
    program_handle = fopen(program_file, "r");
	if (!program_handle) { printf("ERROR: unable to open '%s'\n", program_file); return -1; }
    fseek(program_handle, 0, SEEK_END);
    szKernelLength = ftell(program_handle);
    rewind(program_handle);
    cSources = (char *) malloc(szKernelLength + 1);
    cSources[szKernelLength] = '\0';
    fread(cSources, sizeof(char), szKernelLength, program_handle);
    fclose(program_handle);
		
	// OpenCL initialization
	if(initialize(/* use_gpu */ 1)) return -1;

	// compile kernel
	cl_program prog = clCreateProgramWithSource(context, 1, (const char **)&cSources, &szKernelLength, &err);
